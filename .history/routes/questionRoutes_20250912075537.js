...
// RANDOM - Get one random question (returns only text field)
router.get("/random", async (req, res) => {
  try {
    const count = await Question.countDocuments();
    if (count === 0) return res.status(404).json({ message: "No questions available" });

    const random = Math.floor(Math.random() * count);
    const question = await Question.findOne().skip(random);

    // send only the text (frontend expects response.data.text)
